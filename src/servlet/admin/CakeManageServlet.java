...
public class CakeManageServlet extends HttpServlet {
	...
	private static final String CAKELIST_PATH = "cakeManage/cakeList.jsp";// 商品列表页面地址
	private static final String CAKEADD_PATH = "cakeManage/cakeAdd.jsp";// 商品增加页面地址
	private static final String CAKEEDIT_PATH = "cakeManage/cakeEdit.jsp";// 商品修改页面地址
	private static final String CAKEDETAIL_PATH = "cakeManage/cakeDetail.jsp";// 商品详情页面地址
	private static final String CAKEIMGDIR_PATH="images/cake/cakeimg/";//商品图片保存文件夹相对路径
	...
	private void seachCake(HttpServletRequest request, HttpServletResponse response) 
		...
		request.getRequestDispatcher(CAKELIST_PATH).forward(request, response);
	}
	...
	private void cakeUpdate(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(cakeDao.cakeUpdate(cake)) {
			...
			request.getRequestDispatcher(CAKEEDIT_PATH).forward(request, response);
		}
	}
	...
	private void updateImg(HttpServletRequest request, HttpServletResponse response)
		...
		File contextPath=new File(request.getServletContext().getRealPath("/"));
		File dirPath = new File( contextPath,CAKEIMGDIR_PATH);
		if (!dirPath.exists()) {
			dirPath.mkdirs();
		}
		...
		if (flag) {
			imgSrc = CAKEIMGDIR_PATH + imgName;
	...
	private void cakeAddReq(HttpServletRequest request, HttpServletResponse response)
		...
		request.getRequestDispatcher(CAKEADD_PATH).forward(request, response);
	...
	private void cakeAdd(HttpServletRequest request, HttpServletResponse response)
		...
		InputStream inputStream = null;
		OutputStream outputStream = null;
		File dirPath = new File(request.getServletContext().getRealPath("/") + CAKEIMGDIR_PATH);
		if (!dirPath.exists()) {
			dirPath.mkdirs();
		}
		...
		while (iterator.hasNext()) {
			...
			if (fileItem.isFormField()) {
				...
				if (flag) {
					...
					map.put("imgSrc", CAKEIMGDIR_PATH + imgName);
	...
	private void cakeDetail(HttpServletRequest request, HttpServletResponse response)
		...
		request.getRequestDispatcher(CAKEDETAIL_PATH).forward(request, response);
